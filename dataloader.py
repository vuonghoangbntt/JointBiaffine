...
def get_character(word, max_char_len):
    word_seq = []
    ...
    def get_sample(self):
        for i, sample in enumerate(self.list_sample):
            ...
            for word in sentence:
                character = get_character(word, self.max_char_len)
...
class MyDataSet(Dataset):
    ...
    def __init__(self, path, char_vocab_path, intent_label_path, slot_label_path,
        ...
        self.max_char_len = max_char_len
        with open(slot_label_path, 'r', encoding='utf8') as f:
            self.slot_label_set = f.read().splitlines()

        with open(intent_label_path, 'r', encoding='utf8') as f:
            self.intent_label_set = f.read().splitlines()

        with open(char_vocab_path, 'r', encoding='utf-8') as f:
            self.char_vocab = json.load(f)
        self.slot_2int = {w: i for i, w in enumerate(self.slot_label_set)}
        self.intent_2int = {w: i for i, w in enumerate(self.intent_label_set)}
